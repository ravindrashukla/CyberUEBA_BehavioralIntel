...
def build():
    ...
    add_body_text(doc, (
        ...
        "approach proven in cybersecurity and defense demand forecasting, and applied in "
        "supply-chain risk, to equipment health monitoring. Each piece of equipment becomes "
        "a digital twin — a living, multi-dimensional behavioral profile built from its "
        "operating history across many sensor dimensions and tracked as it evolves over time. "
        "BEI reads how equipment behaves — not just whether individual readings are within "
        "spec — using entity profiles, temporal trajectory and regime analysis, relationship "
        "dynamics, and multi-phase scoring to detect degradation weeks before failure."
    ))
    add_body_text(doc, (
        "The same principle that exposed hidden cyber threats and sharpened defense demand "
        "forecasts applies to equipment: detect the change in behavioral DIRECTION, not the "
        "magnitude of any single reading. BEI brings the representational power of large "
        "language models — the rich semantic space a transformer learns — into the "
        "equipment-reliability application layer, and couples it with rigorous behavioral "
        "mathematics: relationship-drift geometry, trajectory and regime detection, and "
        "change-point analysis. Where an LLM uses that space to understand language, BEI uses "
        "it to understand how equipment is behaving."
    ))
