...
def build():
    ...
    add_callout(doc, "REAL EMBEDDINGS ARE MANDATORY: There is no mock or offline embedder. "
        "Every pipeline calls OpenAI text-embedding-3-small directly and hard-fails without "
        "OPENAI_API_KEY set. Semantically meaningful similarity (~0.3-0.7 for related texts) "
        "is required for detection logic to function; deterministic/random surrogate vectors "
        "cannot validate it.")
    ...
    add_callout(doc, "DETECTION SCOPE: The embedding/composite scorer cleanly separates "
        ...
        "that catches all 4 also admits false positives (~8.5% FP at the 90th percentile). "
    ...
    add_body_text(doc, (
        "Multi-front threat-profile detector (threat_profile_detector.py): a label-free, "
        "standalone detector that matches each entity against measurable known-bad behavioral "
        "profiles rather than relying on embedding drift. Profiles include C2-beacon, DGA-DNS, "
        "LOTL-process, cohort-rare access, recon-fanout, and insider-collection. Each profile "
        "is scored on two fronts — cohort-relative (entity vs. its role-group peers, robust IQR "
        "z-score) and raw-event (direct fingerprints such as beacon periodicity, DGA entropy, "
        "and cohort-rare external destinations) — with self-drift used only as corroboration. "
        "On the 250-user / 485-day dataset this detector flags all four injected attacks "
        "(USR-156 insider-collection, USR-234 C2-beacon/DGA-DNS, USR-042 LOTL-process, "
        "USR-118 recon-fanout/cohort-rare access) at zero false positives."
    ))
    ...
    create_table(doc,
        ...
        [
            ...
            ["SIM_END", "2026-05-01"],
            ["SIM_DAYS", "485 (computed: SIM_END - SIM_START)"],
            ["N_USERS", "250"],
            ["N_DEVICES", "400"],
            ["N_SEGMENTS", "25"],
            ["N_APPLICATIONS", "60"],
