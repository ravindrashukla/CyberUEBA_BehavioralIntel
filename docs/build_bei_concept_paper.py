...
def generate_bei_figures():
    ...
    NV, BL, TL, GR = "#0D1B2A", "#1B4F72", "#0E6B8A", "#1E8A49"

    # 1 — nine dimensions (3x3 grid)
    try:
        dims = ["Identity", "Behavior", "Time", "Space", "Relationship",
                "Network", "Sequence", "Periodicity", "Context"]
        fig, ax = plt.subplots(figsize=(8.4, 3.2)); ax.axis("off"); ax.set_xlim(0, 3); ax.set_ylim(0, 3)
        ax.text(1.5, 3.12, "The Nine Dimensions of Behavioral Entity Intelligence",
                ha="center", fontsize=10, color=NV, weight="bold")
        for i, d in enumerate(dims):
            r, c = divmod(i, 3)
            ax.add_patch(FancyBboxPatch((c + 0.06, (2 - r) + 0.06), 0.88, 0.88,
                                        boxstyle="round,pad=0.02", fc=TL, ec="none"))
            ax.text(c + 0.5, (2 - r) + 0.5, f"{i+1}. {d}", ha="center", va="center",
                    color="white", fontsize=9, weight="bold")
        p = _os.path.join(FIGDIR, "fig_bei_dims.png"); fig.savefig(p, dpi=150, bbox_inches="tight")
        plt.close(fig); figs["dims"] = p
    except Exception:
        pass

    # 2 — three-layer architecture
    try:
        fig, ax = plt.subplots(figsize=(8.0, 3.4)); ax.axis("off"); ax.set_xlim(0, 10); ax.set_ylim(0, 6)

        def lyr(y, t, sub, c):
            ax.add_patch(FancyBboxPatch((1, y), 8, 1.4, boxstyle="round,pad=0.02", fc=c, ec="none"))
            ax.text(5, y + 1.0, t, ha="center", color="white", fontsize=9.5, weight="bold")
            ax.text(5, y + 0.45, sub, ha="center", color="white", fontsize=7)
        lyr(0.3, "Layer 1 — Entity profiles", "per-entity zones, embedded, drift over time", BL)
        lyr(2.1, "Layer 2 — Relationship dynamics", "pairwise (Hadamard) interaction drift", TL)
        lyr(3.9, "Layer 3 — Network intelligence", "graph: multi-hop chains, co-drift, cascade", GR)
        for y in (1.7, 3.5):
            ax.annotate("", xy=(5, y + 0.4), xytext=(5, y), arrowprops=dict(arrowstyle="->", color="#555", lw=1.3))
        p = _os.path.join(FIGDIR, "fig_bei_layers.png"); fig.savefig(p, dpi=150, bbox_inches="tight")
        plt.close(fig); figs["layers"] = p
    except Exception:
        pass

    # 3 — five-phase composite scoring
    try:
        phases = ["Signal strength", "Breadth", "Sustained deviation", "Context divergence", "Novelty persistence"]
        fig, ax = plt.subplots(figsize=(8.6, 3.2)); ax.axis("off"); ax.set_xlim(0, 10); ax.set_ylim(0, 5)
        for i, ph in enumerate(phases):
            y = 3.8 - i * 0.82
            ax.add_patch(FancyBboxPatch((0.3, y), 4.0, 0.65, boxstyle="round,pad=0.02", fc=TL, ec="none"))
            ax.text(2.3, y + 0.32, ph, ha="center", va="center", color="white", fontsize=8)
            ax.annotate("", xy=(6.0, 2.3), xytext=(4.4, y + 0.32), arrowprops=dict(arrowstyle="->", color="#999", lw=1))
        ax.add_patch(FancyBboxPatch((6.0, 1.7), 3.4, 1.2, boxstyle="round,pad=0.02", fc=GR, ec="none"))
        ax.text(7.7, 2.55, "Composite score", ha="center", color="white", fontsize=9.5, weight="bold")
        ax.text(7.7, 2.05, "one ranked verdict", ha="center", color="white", fontsize=7.5)
        ax.text(5, 4.7, "Multi-phase composite: five independent signals -> one ranked score",
                ha="center", fontsize=10, color=NV, weight="bold")
        p = _os.path.join(FIGDIR, "fig_bei_composite.png"); fig.savefig(p, dpi=150, bbox_inches="tight")
        plt.close(fig); figs["composite"] = p
    except Exception:
        pass
...
def build():
    ...
    toc = [
        ...
        "   4.4. Peer-Cohort Baselining — Self vs. Cohort",
    ...
    body(doc, (
        "Mechanically, BEI builds a DIGITAL TWIN of every entity — a living, multi-dimensional "
        "profile of its behavior, its relationships, and how both change over time. It brings the "
        "representational power of large language models — the rich semantic space a transformer "
        "learns — into the application layer of any domain, and couples it with rigorous vector "
        "mathematics: drift geometry, cohort statistics, and change-point detection. Where an LLM "
        "uses that space to understand language, BEI uses it to understand entity behavior."
    ))
    ...
    body(doc, (
        ...
        "what threat pattern. "
        "(All results are synthetic and require re-validation on production data.)"
    ))
    ...
    add_figure(doc, "dims",
               "Figure 1 — The nine complementary dimensions BEI uses to characterize any "
               "observable entity, from identity through context.")
    ...
    add_figure(doc, "layers",
               "Figure 2 — The three analytical layers: entity profiles, pairwise relationship "
               "dynamics, and graph-level network intelligence — each detectable independently.")
    ...
    body(doc, (
        "Concretely, every entity is assembled into a structured, multi-dimensional snapshot — a "
        "JSON-like object whose top-level slots are DIMENSIONS (zones), each holding "
        "SUB-DIMENSIONS (features), captured as a TIME SERIES. The same shape instantiates in "
        "every domain by changing only the zone definitions and what is serialized:"
    ))
    code_block(doc,
        "entity = \"<type>/<id>\"            # one snapshot at time t\n"
        "EntitySnapshot = {\n"
        "  identity:    { static attributes — role/type, class, registration, criticality },\n"
        "  behavior:    { zone_1: {sub-dims}, zone_2: {sub-dims}, ... },   # domain-specific zones\n"
        "  space:       { geographic / network / topological position },\n"
        "  embeddings:  { zone: [k x 1536], composite: [1536], context: [...] },\n"
        "  texts:       { serialized prose per zone }     # audit trail\n"
        "}\n"
        "# stored as a TIME SERIES of snapshots  ->  trajectory (velocity, drift, regime)")
    body(doc, (
        "For a cyber user the zones are identity / access / data / network / risk; for a supplier "
        "they are identity / performance / geographic / network / risk; for a patient, equipment, "
        "or account they are the domain's natural decomposition (Section 3.2). The cadence is the "
        "domain's natural period — weekly for users, monthly for suppliers and equipment — so "
        "every entity is a sequence of embeddings whose trajectory the three layers read."
    ))
    ...
    heading(doc, "4.4. Peer-Cohort Baselining — Self vs. Cohort", level=2)
    body(doc, (
        "A behavioral value is anomalous only in relation to a comparison. BEI uses two "
        "comparisons for every entity, and an entity that fails both is a high-confidence "
        "finding:"
    ))
    bullet(doc, "How does the entity compare to its OWN history? This is the temporal "
           "(Time) baseline — drift from the entity's established pattern.",
           bold_prefix="Self-baseline: ")
    bullet(doc, "How does the entity compare to OTHER entities like it? This is the "
           "peer-cohort baseline — drift relative to the normal behavior of a group of "
           "comparable entities.",
           bold_prefix="Cohort baseline: ")

    body(doc, (
        "Cohorts are formed from the stable attributes of the Identity dimension (Section "
        "3.1): entities are grouped so that members are expected to behave alike, and each "
        "entity's drift is then scored relative to the normal members of its own cohort "
        "rather than the population at large. A behavior that is unremarkable for one cohort "
        "can be a strong signal for another — an action normal for a system administrator is "
        "alarming for an accountant; a multinational sourcing footprint normal for a "
        "semiconductor supplier is alarming for a domestic fastener manufacturer. Scoring "
        "against the wrong reference group hides the signal; scoring against the right cohort "
        "exposes it."
    ))

    callout(doc,
        "Two baselines, one verdict. Self-baseline asks 'is this entity behaving like it "
        "used to?' Cohort baseline asks 'is this entity behaving like its peers?' An entity "
        "drifting from BOTH its own history and its cohort is the strongest anomaly signal "
        "the framework produces."
    )

    body(doc, (
        "Peer-cohort baselining is not a proposal — it is the mechanism already operating in "
        "the cybersecurity proof of concept, where users are segmented into role groups "
        "(administrator, security, developer, business, executive) and each user's drift is "
        "scored relative to the normal baseline of its group. The same construct applies to "
        "every domain by changing only how cohorts are defined:"
    ))

    table(doc,
        ["Domain", "Entity", "Cohort Defined By"],
        [
            ["Cybersecurity", "User",
             "Role group: admin, security, developer, business, executive"],
            ["Supply Chain", "Supplier",
             "Commodity / Federal Supply Class, geography, criticality, tier"],
            ["Healthcare", "Provider",
             "Specialty, practice size, region, patient-population mix"],
            ["Finance / AML", "Account",
             "Product type, customer segment, jurisdiction, expected activity band"],
            ["Maintenance", "Equipment",
             "Model, duty class, age band, operating environment"],
            ["Insurance", "Claimant / Vendor",
             "Policy type, loss category, geography, vendor specialty"],
            ["Counter-Intel", "Person of interest",
             "Role, access tier, cover function, posting / location"],
        ],
        col_widths=[1.2, 1.2, 4.1],
    )

    body(doc, (
        "Cohort baselining also produces a signal of its own: when many members of a cohort "
        "drift together in the same direction, the correlated movement points to a shared "
        "cause — a common sub-tier dependency, a market or geopolitical shock, or a "
        "coordinated campaign — a cohort-level finding that connects directly to the "
        "network-layer cohort-correlation analysis of Section 3.6."
    ))

    heading(doc, "4.5. Mathematical Foundations", level=2)
    body(doc, (
        "The framework reduces to a compact set of operations in a unified vector space; the same "
        "formulas instantiate in every domain by changing only what is serialized into the "
        "embedding. The core operators:"
    ))
    code_block(doc,
        "Behavioral composite (per entity):\n"
        "   V = normalize( SUM_z  w_z * embed( serialize(zone_z) ) )\n\n"
        "Drift between two periods:\n"
        "   magnitude   d_t = 1 - cos( V_{t-1}, V_t )\n"
        "   direction   u_t = ( V_t - V_{t-1} ) / || V_t - V_{t-1} ||\n\n"
        "Direction naming (explainability):\n"
        "   align_k = cos( u_t , embed(concept_k) )   -> rank -> MITRE / domain concept\n\n"
        "Peer-cohort relative score:\n"
        "   z[i,f] = ( x[i,f] - mu_cohort[f] ) / sigma_cohort[f]\n\n"
        "Slow-drift accumulation (CUSUM):\n"
        "   S_0 = 0 ;  S_t = max( 0, S_{t-1} + (d_t - k) ) ;  alarm when S_t > h\n\n"
        "Relationship (pairwise, Hadamard):\n"
        "   R[a x b] = normalize( V_a (*) V_b )")
    body(doc, (
        "Each is domain-agnostic: 'serialize' changes per domain (a user's auth behavior, a "
        "supplier's sourcing mix, a patient's coding pattern), but the geometry — direction over "
        "magnitude, cohort-relative scoring, accumulation, and pairwise composition — is constant."
    ))
    ...
    add_figure(doc, "composite",
               "Figure 3 — Multi-phase composite scoring: five independent signals fused into a "
               "single ranked verdict, so no single evasion technique defeats detection.")
    ...
    body(doc, (
        ...
        "patterns the architecture is designed to surface where traditional analytics fail. "
        "Only the cybersecurity domain (Section 8) is empirically validated to date; the other "
        "eleven are designed applications of the same architecture, not yet validated on data."
    ))
    ...
    body(doc, (
        ...
        "compared to traditional analytical approaches. All figures below are from "
        "SYNTHETIC telemetry and must be re-validated on real operational data before any "
        "production claim."
    ))
    ...
    table(doc,
        ...
        [
            ...
            ["Total event volume", "~14 million events across 5 log sources (485 days)"],
    ...
    callout(doc,
        "Traditional anomaly detection catches at most one of the four attackers (Z-Score 1; "
        "LOF, IForest, OCSVM 0) — and none provide directional intelligence for triage. BEI's multi-phase "
        "composite scoring matches 4/4 detection AND adds zone-level explanation of which "
        "behavioral dimension changed, enabling analysts to distinguish insider from APT from LOTL."
    )

    heading(doc, "8.1. Relation to Prior Art and Defensible Novelty", level=2)
    body(doc, (
        ...
        "combines all elements of the BEI framework. Commercial UEBA (Exabeam, Securonix, "
        "Microsoft Sentinel UEBA) detects via statistical / p-value profiling, machine-learning "
        "baselines, rarity- and magnitude-based scores, and TF-IDF peer grouping; their "
        "generative-AI features are copilots, not the detection mechanism. Commercial demand / "
        "supply-chain platforms (SAP IBP, o9, Kinaxis) forecast with gradient-boosted trees and "
        "machine learning over tabular features on knowledge-graph data models, with generative AI "
        "as a conversational layer."
    ))
    body(doc, (
        "BEI is explicit about prior art: its constituent techniques are individually established — "
        "LLM-embedding of serialized behavior for anomaly detection (APT-LLM, arXiv:2502.09385; "
        "LogBERT, arXiv:2103.04475), serializing raw metrics to prose before embedding (TabLLM "
        "family, arXiv:2502.11596), embed-then-detect pipelines (TAD-Bench, arXiv:2501.11960), and "
        "embedding drift by cosine distance (ZEDD, arXiv:2601.12359). Each, however, detects by "
        "MAGNITUDE and operates in a single domain; none projects the DIRECTION of drift onto "
        "named natural-language concepts, nor unifies multiple domains in one embedding space."
    ))
    callout(doc,
        "To the best of our knowledge — scoped to publicly described and marketed methodology — no "
        "commercial platform and no single published system represents entities of every kind in "
        "one semantic space and detects by the DIRECTION of behavioral drift against named "
        "concepts. BEI's contribution is the integrated, cross-domain architecture and the "
        "concept-direction paradigm, not any single technique. (A formal patent / "
