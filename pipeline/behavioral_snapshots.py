...
embeds via OpenAI Embedder (or MockEmbedder fallback), composes under 4 contexts,
...
from embeddings.embedder import MockEmbedder, Embedder
...
def main():
    ...
    if args.embedder == "openai" or (args.embedder == "auto" and os.environ.get("OPENAI_API_KEY")):
        embedder = Embedder()
        print("Using OpenAI embedder (text-embedding-3-small)", flush=True)
    else:
        embedder = MockEmbedder()
        print("Using MockEmbedder (deterministic, no API key needed)", flush=True)
